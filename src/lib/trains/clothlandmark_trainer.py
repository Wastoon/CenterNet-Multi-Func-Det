...
class clothLandmarkLoss(nn.Module):
    def __init__(self, opt):
        super(clothLandmarkLoss, self).__init__()
        self.crit = FocalLoss()
        self.crit_hm_hp = nn.MSELoss() if opt.mse_loss else FocalLoss()
        self.crit_kp = RegWeightedL1Loss() if not opt.dense_hp else nn.L1Loss(reduction='sum') #####Why sum????
        ...
        for s in range(opt.num_stacks):
            ...
            if opt.dense_hp:
                ...
                hp_loss += (self.crit_kp(output['hps'] * batch['dense_hps_mask'],
                                        batch['dense_hps']* batch['dense_hps_mask']) / mask_weight) / opt.num_stacks
            else:
                hp_loss += self.crit_kp(output['hps'],
                                        batch['hps_mask'],
                                        batch['ind'],
            ...
            if opt.reg_hp_offset and opt.off_weight > 0:
                hp_offset_loss += self.crit_reg(output['hp_offset'],
                                                batch['hp_mask'],
                                                batch['hp_ind'],
...
class clothLandmarkTrainer(BaseTrainer):
    def __init__(self, opt, model, optimizer=None):
        super(clothLandmarkTrainer, self).__init__(opt, model, optimizer=optimizer)
    ...
    def debug(self, batch, output, iter_id):
        ...
        dets[:, :, 5:593] *= opt.input_res / opt.output_res
        dets_gt = batch['meta']['gt_det'].numpy().reshape(1, -1, dets.shape[2])
        dets_gt[:, :, :4] *= opt.input_res / opt.output_res
        dets_gt[:, :, 5:593] *= opt.input_res / opt.output_res
        for i in range(1):
            debugger = Debugger(
            ...
            for k in range(len(dets[i])):
                if dets[i, k, 4] > opt.center_thresh:
                    ...
                    debugger.add_coco_hp(dets[i, k, 5:593], img_id='out_pred')
            ...
            for k in range(len(dets_gt[i])):
                if dets_gt[i, k, 4] > opt.center_thresh:
                    ...
                    debugger.add_coco_hp(dets_gt[i, k, 5:593], img_id='out_gt')

            if opt.hm_hp:
                pred = debugger.gen_colormap_hp(output['hm_hp'][i].detach().cpu().numpy())
                part_perd = debugger.gen_colormap_part_hp_debug(
                    output['hm_hp'][i].detach().cpu().numpy(), part_num=10
                )

                gt = debugger.gen_colormap_hp(batch['hm_hp'][i].detach().cpu().numpy())
                part_gt = debugger.gen_colormap_part_hp_debug(
                    batch['hm_hp'][i].detach().cpu().numpy(), part_num=10
                )

                debugger.add_blend_img(img, pred, 'pred_hmhp')
                debugger.add_blend_img(img, part_perd, 'part_pred_hmhp')
                debugger.add_blend_img(img, gt, 'gt_hmhp')
                debugger.add_blend_img(img, part_gt, 'part_gt_hmhp')
    ...
    def save_result(self, output, batch, results):
        ...
        dets = dets.detach().cpu().numpy().reshape(1 ,-1, dets.shape[2])
        dets_out = clothlandmark_post_process(
            dets.copy(),
    ...
    def _get_losses(self, opt):
        loss_states = ['loss', 'hm_loss', 'hp_loss', 'hm_hp_loss', 'hp_offset_loss', 'wh_loss', 'off_loss']
        loss = clothLandmarkLoss(opt)
