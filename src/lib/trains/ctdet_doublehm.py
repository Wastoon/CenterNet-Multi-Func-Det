...
class CtdetLoss_doublehm(torch.nn.Module):
    ...
    def forward(self, outputs, batch):
        ...
        for s in range(opt.num_stacks):
            ...
            hm_w_loss += self.crit(output['hm_w'], batch['hm_w']) / opt.num_stacks
            hm_h_loss += self.crit(output['hm_h'], batch['hm_h']) / opt.num_stacks
            if opt.wh_weight > 0:
                if opt.dense_wh:
            ...
            loss_stats = {'loss': loss, 'hm_w_loss': hm_w_loss,'hm_h_loss': hm_h_loss,
                          'wh_loss': wh_loss, 'off_loss': off_loss}
        return loss, loss_stats


class CtdetTrainer_Doublehm(BaseTrainer):
    def __init__(self, opt, model, optimizer=None):
        super(CtdetTrainer_Doublehm, self).__init__(opt, model, optimizer=optimizer)

    def _get_losses(self, opt):

        loss_states = ['loss', 'hm_w_loss','hm_h_loss', 'wh_loss', 'off_loss']
        loss = CtdetLoss_doublehm(opt)
    ...
    def debug(self, batch, output, iter_id):
        ...
        for i in range(1):
            ...
            gt = debugger.gen_colormap(batch['hm_h'][i].detach().cpu().numpy())
