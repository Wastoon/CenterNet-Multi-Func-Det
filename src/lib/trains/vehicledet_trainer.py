...
class VehicledetTrainer(BaseTrainer):
    def __init__(self, opt, model, optimizer=None):
        super(VehicledetTrainer, self).__init__(opt, model, optimizer=optimizer)
    ...
    def debug(self, batch, output, iter_id):
        ...
        for i in range(1):
            ...
            if opt.use_contour_fourier:
                ...
                ##1. center_xy up scale to output_res:
                cx = torch.tensor((dets_gt[:, :, 2] - dets_gt[:, :, 0])).unsqueeze(2)
                cy = torch.tensor((dets_gt[:, :, 3] - dets_gt[:, :, 1])).unsqueeze(2)
                center_xy_coordinate_gt = torch.cat([cx, cy], dim=2)
                center_xy_coordinate_gt[:, :, :2] *= opt.down_ratio
                ...
                for k in range(len(dets_gt[i])):
                    if dets_gt[i, k, 5] > opt.center_thresh:
                        debugger.add_dft_points_on_img(dets_gt[i, k, -1], output_res_curves_gt[i,k], img_id='fourier_dft_pred')
