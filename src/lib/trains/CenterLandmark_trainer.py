...
class CenterLandmarkLoss(nn.Module):
...
class CenterLandmarkTrainer(BaseTrainer):
    def __init__(self, opt, model, optimizer=None):
        super(CenterLandmarkTrainer, self).__init__(opt, model, optimizer=optimizer)
    ...
    def debug(self, batch, output, iter_id):
        ...
        dets[:, :, 5:141] *= opt.input_res / opt.output_res
        dets_gt = batch['meta']['gt_det'].numpy().reshape(1, -1, dets.shape[2])
        dets_gt[:, :, :4] *= opt.input_res / opt.output_res
        dets_gt[:, :, 5:141] *= opt.input_res / opt.output_res
        for i in range(1):
            debugger = Debugger(
            ...
            for k in range(len(dets[i])):
                if dets[i, k, 4] > opt.center_thresh:
                    ...
                    debugger.add_coco_hp(dets[i, k, 5:141], img_id='out_pred')
            ...
            for k in range(len(dets_gt[i])):
                if dets_gt[i, k, 4] > opt.center_thresh:
                    ...
                    debugger.add_coco_hp(dets_gt[i, k, 5:141], img_id='out_gt')
