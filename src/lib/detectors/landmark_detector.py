...
import torch
import time
from models.utils import landmark_flip_lr_off, flip_tensor, flip_lr
from models.decode import landmark_decode
...
class LandmarkDetector(BaseDetector):
        ...
        with torch.no_grad():
            ...
            if self.opt.flip_test:
                ...
                output['hps'] = (output['hps'][0:1] +
                                 landmark_flip_lr_off(output['hps'][1:2], self.flip_idx)) / 2
            ...
            dets = landmark_decode(
                ...
                reg=reg, hm_hp=hm_hp, hp_offset=hp_offset, K=self.opt.K, reg_only=False)
    ...
    def post_process(self, dets, meta, scale=1):
        ...
        for j in range(1, self.num_classes + 1):
            dets[0][j] = np.array(dets[0][j], dtype=np.float32).reshape(-1, 141)
    ...
    def debug(self, debugger, images, dets, output, scale=1):
        ...
        dets[:, :, 5:141] *= self.opt.down_ratio
        ...
        if self.opt.hm_hp:
            ...
            hm_nose = debugger.gen_colormap_specific_hp_debug(
                output['hm_hp'][0].detach().cpu().numpy(),  point_num=30
            )

            debugger.add_blend_img(img, part_perd, 'part_pred_hmhp')
            debugger.add_blend_img(img, hm_nose, 'nose_hmhp')
    ...
    def show_results(self, debugger, image, results):
        ...
        for bbox in results[1]:
            if bbox[4] > self.opt.vis_thresh:
                ...
                debugger.add_coco_hp(bbox[5:141], img_id='landmark')
                #debugger.add_hp_offset(bbox[5:141], img_id='landmark')
