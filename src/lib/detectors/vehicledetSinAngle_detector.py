...
class VehicledetSinAngleDetector(BaseDetector):
    def __init__(self, opt):
        super(VehicledetSinAngleDetector, self).__init__(opt)
            ...
            for k in range(len(dets[i])):
                if detection[i, k, 5] > self.opt.center_thresh:
                    ...
                    debugger.add_coco_Rotate_bbox(detection[i, k, :4], detection[i, k, -1],
                                                  detection[i, k, 4], detection[i, k, 5], img_id='angled_pred_out', system='Rad')
