...
class RegL1loss_gridneighbor(nn.Module):
    ...
    def forward(self, output, mask, ind, target, mainpoints_list):
        ...
        mask = mask.unsqueeze(3).expand(mask.size(0),mask.size(1),mask.size(2),pred.size(2)//mask.size(2)).contiguous().view(mask.size(0),mask.size(1),-1).float()
        # loss = F.l1_loss(pred * mask, target * mask, reduction='elementwise_mean')
        loss = 0
        for point in range(len(mainpoints_list)):
            loss += F.l1_loss(pred[:,:,point:point+2] * mask[:,:,point:point+2], target[:,:,point:point+2] * mask[:,:,point:point+2], size_average=False)
        loss = loss / (mask.sum() + 1e-4)
        return loss
