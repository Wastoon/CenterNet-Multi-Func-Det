...
def vehicledet_decode(heat, wh, ang, reg=None, cat_spec_wh=False, K=100):
    batch, cat, height, width = heat.size()
    ...
    ang = _tranpose_and_gather_feat(ang, inds)
...
def vehicledet_fourier_contour_decode(heat, wh, ang, reg=None, cat_spec_wh=False, K=100):
    batch, cat, height, width = heat.size()
    ...
    center_xy_coordinate = torch.cat([xs, ys], dim=2) ## batch x K x 2
    detections = torch.cat([bboxes, scores, clses], dim=2)

    return detections, center_xy_coordinate, inds
...
def decode_batch_curve(fourier_contour_map, center_index, down_ratio, order=100, time_length=300, K=100):
    extract_contour = fourier_contour_decode(fourier_contour_map, center_index, order=order)

    extract_contour = de_scale_fourier_coef(extract_contour, down_ratio)
    extract_contour = de_translation_fourier_coef(extract_contour, 0.0, 0.0)
    ...
    for i in range(batch):
        for j in range(K):
            ...
            assert coef_list.shape[0] == (2*order+1)
...
def decode_batch_curve_gt(extract_contour, down_ratio, order=100, time_length=300, K=100):

    extract_contour = de_scale_fourier_coef(extract_contour, down_ratio)
    extract_contour = de_translation_fourier_coef(extract_contour, 0.0, 0.0)

    batch = extract_contour.shape[0]
    decoded_contour = np.zeros((batch, K, time_length, 2))

    for i in range(batch):
        for j in range(K):
            coef_list = extract_contour[i][j].reshape(-1, 2)
...
def trans_curve2_obj_center(decoded_contour, center_xy_coordinate):
    ...
    center_xy_coordinate = center_xy_coordinate.unsqueeze(2).numpy()
...
def ctdet_decode_decodeWH_from_hm(heat, wh, reg=None, cat_spec_wh=False, K=100):
    batch, cat, height, width = heat.size()
...
def gridneighbor_bbox_decode(xs, ys, wh, point_flags):
    ...
    for idx, point in enumerate(point_flags):
        ...
        x1 = (xs_i - w_i/2).unsqueeze(2)
        y1 = (ys_i - h_i/2).unsqueeze(2)
        x2 = (xs_i + w_i/2).unsqueeze(2)
        y2 = (ys_i + h_i/2).unsqueeze(2)
