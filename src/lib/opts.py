...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--task', default='gridneighbordet',
                             help='ctdet | ddd | multi_pose | exdet|gridneighbordet')
    self.parser.add_argument('--dataset', default='coco',
                             help='coco | kitti | coco_hp | pascal')
    self.parser.add_argument('--exp_id', default='gridneighbordet-debug-loss')
    ...
    self.parser.add_argument('--demo', default='/data/mry/DataSet/COCO/val2014',
                             help='path to image/ image folders/ video. '
                                  'or "webcam/data/mry/DataSet/COCO/val2014/COCO_val2014_000000017882.jpg"')
    self.parser.add_argument('--load_model', default='/data/mry/code/CenterNet/exp/gridneighbordet/coco-gridneighbordet-pretrained-singlemainpoint-8/model_best.pth',
                             help='path to pretrained model, like ')
    self.parser.add_argument('--det_output_path', default='',
                             help='path to save det results')
    ...
    self.parser.add_argument('--logger_iteration', type=int, default=1,
                             help='step of logger in iteration.')
    ...
    # landmark_300w
    self.parser.add_argument('--test_split', default= 'full',
                             help='300w Dataset test split: common | challenge | full')
    self.parser.add_argument('--reg_or_reghm', default=False,
                             help='use reghm or reg_only to post process the det results')
    self.parser.add_argument('--keep_inp_res_prob', type=float, default=0,
                             help='As for the input resolution, should be the img origial res or a fixed res')
    # ddd
    ...
    self.parser.add_argument('--reconstruct', action='store_true',
                             help='use reconstruct loss or not.')
    self.parser.add_argument('--test_reconstruct_conflict_with_class', action='store_true',
                             help='debug reconstruct conflict with class or not.')
    # ctdet
    ...
    self.parser.add_argument('--reg_sigma_offset_weight', type=float, default=1,
                             help='loss weight for keypoint sigma_offset_weight.')
    ...
    self.parser.add_argument('--not_reg_sigmawh_offset', action='store_true',
                             help='not regress local sigmawh offset.')
    # exdet
  ...
  def parse(self, args=''):
    ...
    opt.point_flags = [int(i) for i in opt.point_flags.split(',')]

    opt.fix_res = not opt.keep_res
    print('Fix size testing.' if opt.fix_res else 'Keep resolution testing.')
    opt.reg_offset = not opt.not_reg_offset
    opt.reg_sigmawh_offset = not opt.not_reg_sigmawh_offset
  ...
  def update_input_res(self, opt, new_width, new_height):
    opt.input_h = new_height
    opt.input_w = new_width
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    if opt.task == 'exdqet':
      ...
      opt.heads = {'hm': opt.num_classes * len(opt.point_flags),
                   'wh': 2* len(opt.point_flags) if not opt.cat_spec_wh else 2 * opt.num_classes}
      if opt.reg_offset:
        opt.heads.update({'reg': 2* len(opt.point_flags)})
    elif opt.task == 'ctdet_esphm':
      # assert opt.dataset in ['pascal', 'coco']
      opt.heads = {'hm': opt.num_classes * len(opt.point_flags)}
      if opt.reg_offset:
        opt.heads.update({'reg': 2* len(opt.point_flags)})
      if opt.reg_sigmawh_offset:
        opt.heads.update({'reg_sigmawh_offset': 2* len(opt.point_flags)})

    elif opt.task == 'ctdet_Dhm':
      # assert opt.dataset in ['pascal', 'coco']
      opt.heads = {'hm_w': opt.num_classes,'hm_h': opt.num_classes,
                   'wh': 2 if not opt.cat_spec_wh else 2 * opt.num_classes}
      if opt.reg_offset:
        opt.heads.update({'reg': 2* len(opt.point_flags)})
        
    elif opt.task == 'gridneighbordet':
  # assert opt.dataset in ['pascal', 'coco']
      opt.heads = {'hm': opt.num_classes * len(opt.point_flags),
               'wh': 2* len(opt.point_flags) if not opt.cat_spec_wh else 2 * opt.num_classes}
      if opt.reg_offset:
      ...
      if opt.reg_hp_offset:
        ...
        # assert opt.dataset in ['coco_hp']
        opt.flip_idx = dataset.flip_idx
        opt.heads = {'hm': opt.num_classes, 'wh': 2, 'hps': 136}
        if opt.reg_offset:
            opt.heads.update({'reg': 2})
        if opt.hm_hp:
            opt.heads.update({'hm_hp': 68})
        if opt.reg_hp_offset:
            opt.heads.update({'hp_offset': 2})
        ...
        # assert opt.dataset in ['coco_hp']
        opt.flip_idx = dataset.flip_idx
        opt.heads = {'hm': opt.num_classes, 'wh': 2, 'hps': 588}
        if opt.reg_offset:
            opt.heads.update({'reg': 2})
        if opt.hm_hp:
  ...
  def init(self, args=''):
    default_dataset_info = {
      'ctdet': {'default_resolution': [512, 512], 'num_classes': 80,
                ...
                'dataset': 'coco'},
    'ctdet_Dhm': {'default_resolution': [512, 512], 'num_classes': 80,
              'mean': [0.408, 0.447, 0.470], 'std': [0.289, 0.274, 0.278],
              'dataset': 'coco'},
    'ctdet_esphm': {'default_resolution': [512, 512], 'num_classes': 80,
              'mean': [0.408, 0.447, 0.470], 'std': [0.289, 0.274, 0.278],
              'dataset': 'coco'},
      'gridneighbordet': {'default_resolution': [512, 512], 'num_classes': 80,
        'mean': [0.408, 0.447, 0.470], 'std': [0.289, 0.274, 0.278],
        'dataset': 'coco'},
      ...
      'ddd': {'default_resolution': [384, 1280], 'num_classes': 3,
        ...
        'landmark': {
            ...
            'flip_idx': [[0, 16], [1, 15], [2, 14], [3, 13], [4, 12], [5, 11], [6, 10], [7, 9], [17, 26], [18, 25],
                         ...
                         [67, 65]]},
        'cloth': {
            'default_resolution': [256,256], 'num_classes': 13,
            'mean': [0.408, 0.447, 0.470], 'std': [0.289, 0.274, 0.278],
            'dataset': 'deepfashion2', 'num_joints': 294,
            'flip_idx': [[0, 16], [1, 15], [2, 14], [3, 13], [4, 12], [5, 11], [6, 10], [7, 9], [17, 26], [18, 25],
                         [19, 24], [20, 23], [21, 22], [39, 42], [38, 43], [37, 44], [36, 45], [41, 46], [40, 47],
                         [31, 35], [32, 34], [48, 54], [49, 53], [50, 52], [59, 55], [58, 56], [60, 64], [61, 63],
                         [67, 65]]},
