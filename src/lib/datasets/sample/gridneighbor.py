...
class GridNeighborDetDataset(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    wh = np.zeros((self.max_objs, 2*main_points_num), dtype=np.float32)
    ...
    for k in range(num_objs):
      ...
      if h > 0 and w > 0:
        ...
        for idx, main_points in enumerate(main_points_list):
            ...
            w1, h1 = wh[k] [2*idx:2*idx+2]
            gt_det.append([ct[0] - w1, ct[1] - h1, 
               ct[0] + w1*(3**(1-col)), ct[1] + h1*(3**(1-row)), 1, cls_id])
            #test_list.append((ct_int, w,h, w1, h1))
